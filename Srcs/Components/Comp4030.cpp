...
{
	std::cout << "I am a Component 4030 with 4 XOR gates" << std::endl;
}

std::pair<std::size_t, std::size_t> Comp4030::getRelativeInputPins(size_t pin)
{
...
nts::Tristate Comp4030::compute(size_t pin)
{
	if (pin > 14)
		throw (NanoException("The pin " + std::to_string(pin) + " does not exist on a component of type 4030."));
	if (_others[pin - 1] != nullptr) {
		if (getPin(pin) == nts::OUT) {
			...
			_values[pin - 1] = TruthTable::XorTruthTable(_values[pins.first - 1], _values[pins.second - 1]);
		}
...
nts::Pin Comp4030::getPin(size_t pin)
{
	nts::Pin res;
...
void Comp4030::setLink(std::size_t pin, nts::IComponent &oth, std::size_t othPin)
{
	if (pin > 14)
		throw (NanoException("The pin " + std::to_string(pin) + " does not exist on a component of type 4030."));
	if (_others[pin - 1] != nullptr)
