...
{
	std::cout << "I am a Component 4071 with 4 OR gates" << std::endl;
}

std::pair<std::size_t, std::size_t> Comp4071::getRelativeInputPins(size_t pin)
{
...
nts::Tristate Comp4071::compute(size_t pin)
{
	if (pin > 14)
	...
	if (_others[pin - 1] != nullptr) {
		if (getPin(pin) == nts::OUT) {
			...
			_values[pin - 1] = TruthTable::orTruthTable(_values[pins.first - 1], _values[pins.second - 1]);
		}
...
nts::Pin Comp4071::getPin(size_t pin)
{
	nts::Pin res;
...
void Comp4071::setLink(std::size_t pin, nts::IComponent &oth, std::size_t othPin)
{
	if (pin > 14)
		throw (NanoException("The pin " + std::to_string(pin) + " does not exist on a component of type 4071."));
	if (_others[pin - 1] != nullptr)
